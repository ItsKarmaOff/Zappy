    ...
    orientation_t orientation)
{
    ...
    printf("dx: %ld dy: %ld\n", !((long)orientation % 2) * (3 - (long)orientation), ((long)orientation % 2) * ((long)orientation - 2));
    ...
    printf("x: %zu y: %zu\n", player->position.x, player->position.y);
    printf("ejecter: %u ejected: %u\n", orientation, player->orientation);
    if ((long)orientation - (long)player->orientation == 0)
        dprintf(player->client->socket_fd, "eject: %d\n", 5);
    else if ((long)orientation - (long)player->orientation == 2
    || (long)player->orientation - (long)orientation == 2)
        dprintf(player->client->socket_fd, "eject: %d\n", 1);
    else if ((long)orientation - (long)player->orientation == 1
    || (long)player->orientation - (long)orientation == 1)
        dprintf(player->client->socket_fd, "eject: %d\n", 3);
    else if ((long)orientation - (long)player->orientation == 3
    ...
    UNUSED char **args)
{
    ...
    tmp != NULL; tmp = tmp->next) {
        if (((player_t *)tmp->data)->is_egg) {
            ...
            fail = false;
        } else if (tmp->data != client->player) {
